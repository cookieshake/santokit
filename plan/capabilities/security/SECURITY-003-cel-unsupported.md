...
## Intent
Fail fast when CEL expression cannot be safely converted/evaluated in supported path. Receive explicit failure for unsupported condition shape instead of silent policy bypass.
...
- [ ] No partial result or silent bypass occurs when an unsupported operator is encountered.
