...
## Intent
Allow deterministic literal condition checks for resource attributes. Enforce fixed-value resource filters from policy (for example status gates) without client cooperation.
...
## Acceptance Criteria
- [ ] Only rows matching the literal CEL condition are returned.
- [ ] Rows not matching the literal value are not visible to the caller.
