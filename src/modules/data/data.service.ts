...
export const dataService = {
    // Insert Data
    create: async (projectId: number, collectionName: string, data: Record<string, any>) => {
        // 1. Resolve Meta
        ...
        const valueString = values.map(v => {
            if (typeof v === 'string') return `'${v.replace(/'/g, "''")}'` // basic memory-safe escaping
            return v
        }).join(', ')
    ...
    // Query Data
    findAll: async (projectId: number, collectionName: string) => {
        const col = await collectionRepository.findByProjectAndName(projectId, collectionName)
        if (!col) throw new Error('Collection not found')
