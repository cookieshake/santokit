...
export const CollectionDetail = (props: {
  ...
  return (
    <Layout
      title={`Collection: ${props.collectionName}`}
      active="projects"
    ...
    >
      <nav class="breadcrumb">
        <ul>
          ...
          <li>
            <a href={`/ui/projects/${props.projectId}`}>Project {props.projectId}</a>
          </li>
          <li class="is-active">
      ...
      <div class="tabs is-boxed">
        <ul>
          <li class="is-active" data-tab="data-tab" onclick="switchTab('data-tab')">
            <a>Data</a>
          </li>
          <li data-tab="structure-tab" onclick="switchTab('structure-tab')">
            <a>Structure</a>
          </li>
          <li data-tab="policies-tab" onclick="switchTab('policies-tab')">
            <a>Policies</a>
          </li>
        </ul>
      </div>
      ...
      <div id="data-tab" class="tab-content">
        <div class="level">
          ...
          <div class="level-right">
            <div class="level-item">
              <div class="buttons">
                <button class="button is-primary" onclick="showModal('add-row-modal')">
                  Insert Data
                </button>
              </div>
            </div>
        ...
        <div class="box mb-5">
          {props.rows.length === 0 ? (
            <div class="notification">No data yet.</div>
          ) : (
                ...
                <tbody>
                  {props.rows.map((row) => (
                    <tr>
                      {props.detail.fields.map((f: any) => (
                        <td>
                          {row[f.column_name] === null ? (
                            <span class="has-text-grey-light">NULL</span>
                          ) : typeof row[f.column_name] === 'object' ? (
                            JSON.stringify(row[f.column_name])
                          ) : (
      ...
      <div id="structure-tab" class="tab-content" style="display: none;">
        <div class="level">
          ...
          <div class="level-right">
            <div class="level-item">
              <div class="buttons">
                <button class="button is-link" onclick="showModal('add-field-modal')">
                  Add Field
                </button>
              </div>
            </div>
                  ...
                  <tbody>
                    {props.detail.fields.map((field: any) => (
                      <tr>
                        ...
                        <td>
                          <button
                            onclick={`deleteField('${field.column_name}')`}
                ...
                <table class="table is-fullwidth is-hoverable">
                  ...
                  <tbody>
                    {props.policies.map((policy: any) => (
                      <tr>
                        <td>
                          <span class="tag is-info is-light">{policy.role}</span>
                        </td>
                        <td>{policy.action}</td>
                        <td>
                          <code class="is-size-7">{policy.condition}</code>
                        </td>
                        <td>
                          <span
                            class={`tag ${policy.effect === 'allow' ? 'is-success' : 'is-danger'}`}
                          >
                            {policy.effect}
                          </span>
                        </td>
            ...
            <div class="box">
              ...
              <form id="create-policy-form">
                ...
                <div class="field">
                  ...
                  <p class="help">
                    Use <code>$user.id</code> to reference the current user's ID.
                  </p>
                </div>
                <div class="field">
        ...
        <div class="modal-card" style="width: 90%;">
          ...
          <footer class="modal-card-foot">
            <button
              ...
              onclick="document.getElementById('add-row-form').requestSubmit()"
            >
              Insert
            </button>
            <button class="button" onclick="hideModal('add-row-modal')">
              Cancel
            </button>
          </footer>
        </div>
      ...
      <div id="add-field-modal" class="modal">
        ...
        <div class="modal-card">
          ...
          <footer class="modal-card-foot">
            <button
              class="button is-link"
              onclick="document.getElementById('add-field-form').requestSubmit()"
            >
              Add
            </button>
            <button class="button" onclick="hideModal('add-field-modal')">
              Cancel
            </button>
          </footer>
        </div>
