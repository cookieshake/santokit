...
app.get('/projects/:id', async (c) => {
  ...
  try {
    ...
    if (databases.length > 0) {
      ...
      const selectedDb = dbNameParam
        ? databases.find((d) => d.name === dbNameParam) || databases[0]
        : databases[0]
...
app.get('/projects/:id/collections/:colName', async (c) => {
  ...
  try {
    ...
    const selectedDb = dbNameParam
      ? databases.find((d: any) => d.name === dbNameParam) || databases[0]
      : databases[0]
    ...
    return c.html(
      <Layout title="Error" active="projects" account={c.get('account')}>
        <div class="notification is-danger">Error: {String(e)}</div>
      </Layout>,
    )
  }
})
