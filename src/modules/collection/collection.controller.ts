...
app.post('/:collectionName/fields', zValidator('json', AddFieldSchema), async (c) => {
    try {
        ...
        const dryRun = c.req.query('preview') === 'true'
        const result = await collectionService.addField(databaseId, collectionName, name, type, !!isNullable, dryRun)
...
app.put('/:collectionName/fields/:fieldName', zValidator('json', RenameFieldSchema), async (c) => {
    try {
        ...
        const dryRun = c.req.query('preview') === 'true'
        const result = await collectionService.renameField(databaseId, collectionName, oldName, newName, dryRun)
        if (dryRun) return c.json(result)
...
app.delete('/:collectionName/fields/:fieldName', async (c) => {
    try {
        ...
        const dryRun = c.req.query('preview') === 'true'
        const result = await collectionService.removeField(databaseId, collectionName, fieldName, dryRun)
        if (dryRun) return c.json(result)
...
app.post('/:collectionName/indexes', zValidator('json', CreateIndexSchema), async (c) => {
    try {
        ...
        const dryRun = c.req.query('preview') === 'true'
        const result = await collectionService.createIndex(databaseId, collectionName, indexName, fields, !!unique, dryRun)
        if (dryRun) return c.json(result)
...
app.delete('/:collectionName/indexes/:indexName', async (c) => {
    try {
        ...
        const dryRun = c.req.query('preview') === 'true'
        const result = await collectionService.removeIndex(databaseId, collectionName, indexName, dryRun)
        if (dryRun) return c.json(result)
