...
func NewLoginCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "login",
		Short: "Login to Santokit Hub via browser.",
		...
		`)
			...
			loginURL := fmt.Sprintf("%s/auth/cli?callback=%s", hubURL, callbackURL)
			...
			userInfo, err := getUserInfo(hubURL, token)
			if err != nil {
	...
	cmd.Flags().StringVar(&hubURL, "hub-url", "https://hub.santokit.dev", "Hub URL")
	return cmd
}
...
func getUserInfo(hubURL, token string) (*userInfoResponse, error) {
	req, err := http.NewRequest("GET", hubURL+"/api/v1/auth/me", nil)
	if err != nil {
		return nil, err
	}
