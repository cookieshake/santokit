...
## 4가지 핵심 엔진
1.  **스캐너 (Scanner)**: santokit 프로젝트 파일의 변경 사항을 감시합니다. `.gitignore` 및 `node_modules`는 무시합니다.
2.  **파서 (Parser)**: 파일에서 메타데이터(YAML)를 추출합니다. "단일 파일"(주석 파싱) 전략을 처리합니다.
...
*   **대상**: 로컬 머신에서 `stk dev`, `stk logic apply` 등을 실행하는 개발자.
*   **동작**:
